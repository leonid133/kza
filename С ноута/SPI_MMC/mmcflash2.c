...
xdata unsigned long address;// = 0x00000200;
...
void SPI_isr(void) interrupt 6
{
	...
	SPIF = 0;
	if((MMC_CMD == 0xFE)||(MMC_CMD == 0xFF))
	{
		SFRPAGE = SFRPAGE_SAVE;
		return;
	}

	if(MMC_CMD == 0x00) //init------------------------
	{
		flash=0;
		if(i_RST==0x00)
		{
