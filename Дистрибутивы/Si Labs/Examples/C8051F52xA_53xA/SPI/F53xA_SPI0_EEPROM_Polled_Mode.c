...
//
// Method1:
// 1) Download the code to a 'F53xA device (either device A or device B on the
//    target board) that is connected to a Microchip 4 kB EEPROM 25LC320 as
//    described above.
...
//
// Method2 (optional):
// 1) Download the code to a 'F53xA device (device A on the target board) that
//    is connected to a Microchip 4 kB EEPROM 25LC320 as described above.
// 2) Ensure the UART TX/RX pin jumpers are populated (HDR4).
// 3) Connect a USB cable from P1 on the target board to the PC.
...
void PORT_Init (void)
{
   ...
   XBR0     = 0x03;                    // Enable the UART and the SPI
