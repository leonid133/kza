...
//
// Method1:
// 1) Download the code to a 'F93x device that is connected as above.
...
//
// Method2 (optional):
// 1) Download code to a 'F93x device that is connected as above
...
void EEPROM_Write (UINT address, BYTE value)
{
   ...
   NSS0MD0   = 0;                       // Step1.1: Activate Slave Select
   SPI0DAT  = EEPROM_CMD_WREN;         // Step1.2: Send the WREN command
   while (!SPIF0);                      // Step1.3: Wait for end of transfer
   SPIF0     = 0;                       // Step1.4: Clear the SPI intr. flag
   NSS0MD0   = 1;                       // Step1.5: Deactivate Slave Select
   ...
   do
   {
      NSS0MD0   = 0;                    // Activate Slave Select
      SPI0DAT  = EEPROM_CMD_RDSR;      // Send the Read Status Register command
      while (!SPIF0);                   // Wait for the command to be sent out
      SPIF0     = 0;
      SPI0DAT  = 0;                    // Dummy write to output serial clock
      while (!SPIF0);                   // Wait for the register to be read
      SPIF0     = 0;
      NSS0MD0   = 1;                    // Deactivate Slave Select after read
      Delay_us (1);
...
BYTE EEPROM_Read (UINT address)
{
   ...
   SPI0DAT  = 0;                       // Dummy write to output serial clock
   while (!SPIF0);                      // Wait for the value to be read
   SPIF0     = 0;
   NSS0MD0   = 1;                       // Deactivate Slave Select
   Delay_us (1);
