...
void SPI_isr(void) interrupt 6
{
	...
	static xdata unsigned long timeFlashOver = 0;
 ...
 //-end read
	...
	{
		if(BuffOutKza[0]==0x65 && BuffOutKza[1]==0x6f && BuffOutKza[2] == 0x66 && BuffOutKza[3]==0x65 && BuffOutKza[4]==0x6f && BuffOutKza[5] ==0x66 && BuffOutKza[6]==0x65 && BuffOutKza[7]==0x6f && BuffOutKza[8] ==0x66)
		{
			flash = 1;
			flageofrec = 0;
			MMC_CMD = 0xFF;
...
{
	static xdata unsigned int counterBuf = 9, endZapCount = 0;
	...
	{
		tmpNzap = BuffOutKza[0];
		for(i_wr = 1; i_wr < 4; i_wr++)
		{
			tmpNzap = (tmpNzap<<8)+BuffOutKza[i_wr];
		}
		...
		else  //читать дальше
		{
			...
			if(!CRC) 
			{
				address = BuffOutKza[4];
				address = (address<<8)+BuffOutKza[5];
				address = (address<<8)+BuffOutKza[6];
				address = (address<<8)+BuffOutKza[7];
			}
			else
			{
				address += 0x200;	
	...
	else if((flash == 5)&&(nzap == 0))
	{
		...
		if(!CRC && (BuffOutKza[0]!=0 || BuffOutKza[1]!=0 || BuffOutKza[2]!=0 || BuffOutKza[3]!=0 || BuffOutKza[4]!=0 || BuffOutKza[5]!=0 || BuffOutKza[6]!=0 || BuffOutKza[7]!=0 || BuffOutKza[8]!=0)) 
		{
				nzap = BuffOutKza[0];
				for(i_wr = 1; i_wr < 4; i_wr++)
				{
	...
	{
		if((eofaddr-address)<0x400 || (eofaddr-address) > szflash)
		{
			flageofrec = 1;
